<template>
    <div class="template">
        ...
        <div class="template-content">
            ...
            <!--新增弹出框 -->
            <el-dialog title="新增船型" :visible.sync="addVisible" width="40%">
            ...
            <!-- 编辑弹出框 -->
            <el-dialog title="编辑船型" :visible.sync="editVisible" width="40%">
                <el-form ref="form"  label-width="100px">
                    <el-form-item label="船型名称">
                        <el-input v-model="name" style="width: 200px"></el-input>
                    </el-form-item>
    ...
    export default {
        ...
        methods: {
            ...
            loadingShowData() {
                let that = this;
                axios.all([
                    axios.post(" " + url + "/sys/showTableTitle", {"name": "chuanxingshezhi"}),
                    axios.post(" " + url + "/ship/shipTypeList")
                ])
            ...
            doAdd() {
                if (this.name) {
                    axios.post(" " + url + "/ship/insertShipType",
                        {
                            "name": this.name
                        }
                    )
            ...
            edit(row, column, cell, event) {
                ...
                this.id = row.id;
                axios.post(" " + url + "/ship/getShipTypeDetail", {"id": this.id})
                    .then((res) => {
            ...
            saveEdit() {
                if (this.name) {
                    axios.post(" " + url + "/ship/updateShipType",
                        {
                            "id": this.id,
                            "name": this.name,
                        }
            ...
            // 确定删除
            deleteRow() {
                axios.post(" " + url + "/ship/delShipType",
                    {
                        "id": this.listData[0],
                    }
                )
